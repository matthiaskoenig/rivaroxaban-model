"""Tool to run the rivaroxaban model factory and simulation experiments."""
...
from pkdb_models.models.rivaroxaban import RIVAROXABAN_PATH
from pkdb_models.models.rivaroxaban.simulations import run_simulation_experiments, EXPERIMENTS
from sbmlutils.console import console

FACTORY_SCRIPT_PATH = RIVAROXABAN_PATH / "models" / "factory.py"
...
def _setup_custom_results_paths(results_dir: str):
    ...
    import pkdb_models.models.rivaroxaban as rivaroxaban
    custom_path = Path(results_dir).resolve()
    custom_path.mkdir(parents=True, exist_ok=True)
    # Override the module paths
    rivaroxaban.RESULTS_PATH = custom_path
    rivaroxaban.RESULTS_PATH_SIMULATION = custom_path / "simulation"
...
def _get_current_results_path():
    ...
    from pkdb_models.models.rivaroxaban import RESULTS_PATH
    return RESULTS_PATH
...
def _list_available_experiments():
    ...
    console.print('[dim]Example: run_rivaroxaban --action simulate --experiments "misc,Ding2020"[/dim]')
    console.print('[dim]Or use "all" to run all experiments: run_rivaroxaban --action simulate --experiments all[/dim]\n')
...
def main() -> None:
    ...
    console.rule("[bold cyan]RIVAROXABAN PBPK/PD MODEL[/bold cyan]", style="cyan")
    ...
    if options.results_dir:
        ...
        if options.action and options.action.lower() == "factory":
            console.print(f"[cyan]Factory will use model base path: {rivaroxaban.MODEL_BASE_PATH}[/cyan]")
        else:
...
if __name__ == "__main__":
    """
    This script is intended to be run from the command line as 'run_rivaroxaban'.
    ...
    1. Help:
       ...
       $ run_rivaroxaban --help

    2. Run the Model Factory:
       Generates all SBML model files.
       $ run_rivaroxaban --action factory

    3. Run Simulations:
       List available experiments:
       $ run_rivaroxaban --action list_experiments

       Run all experiments:
       $ run_rivaroxaban --action simulate --experiments all

    5. Run Everything:
       Runs factory and all simulations.
       $ run_rivaroxaban --action all
       
       With custom results directory for figures:
       $ run_rivaroxaban --action all --results-dir '/path/to/my/results'
    """
    main()